...
class Drone6DConditioned(Dynamics):
    def __init__(self):
        ...
        super().__init__(
            ...
            state_var=[2,0.75, 0.5,1.5,1.5,0.3, 0.25],
            value_mean=0.9,
            value_var=1.2,
            value_normto=0.02,
            deepReach_model='reg',
    ...
    def boundary_fn(self, state):
        ...
        dist=torch.maximum(dist, torch.zeros_like(dist)) - self.collision_R
        return torch.minimum(torch.minimum(-torch.abs(state[...,1]+0.25)+0.5, dist),state[...,0])
    def sample_target_state(self, num_samples):
        raise NotImplementedError
    def cost_fn(self, state_traj):
